...
<p align="center">
Command Code Go / GOAT / Pro Plan 订阅接入 DSH，模型列表自动同步，开箱即用。
</p>
...
本插件解决第二种情况：通过 `/alpha/generate` 流式接入 DSH 的原生 `LlmAdapter`，让 Go / GOAT / Pro Plan 用户直接在 DSH 中使用订阅的模型。模型列表不写死在代码里——插件会定时从 `/provider/v1/models` 拉取实时目录，并从官方 CLI catalog (CDN) 合并每个模型的 Reasoning Effort 支持。
...
- 或通过 DSH 的 credentials 服务保存（Web Models 设置页写入）。
