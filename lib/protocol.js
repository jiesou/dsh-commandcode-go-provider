...
function serializeUser(message, images) {
    ...
    if (toolResults.length > 0 && flattenText(message.content).length === 0) {
        ...
        if (nested.length > 0)
            wire.push({ role: 'user', content: nested.map(ref => imagePart(ref, images)) });
        return wire;
    }
    ...
    if (parts.every(part => part.type === 'text')) {
        return [{ role: 'user', content: parts.map(part => part.text).join('') }];
    }
...
export function buildRequest(options, images) {
    ...
    for (const message of options.messages) {
        ...
        if (message.role === 'assistant')
            ...
            messages.push(...serializeUser(message, images));
    }
